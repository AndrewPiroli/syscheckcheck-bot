...
re_cios_title_base_detect = re.compile(r"IOS(\d{1,3})\[(\d{1,3})\]") # Group 1 = tid Group 2 = Base IOS
re_d2x_detect = re.compile(r"d2x-(v\d{1,2})(beta|final)(\d{0,2}(-alt)?)") # Group 1 d2x version "v10" Group 2 beta or final Group 3 beta version
...
def process_syscheck(syscheck_lines: Iterator[str]) -> Tuple[dict, int]:
    ...
    for entry in syscheck_lines:
        ...
        if patch_no_patches_detect in entry:
            ...
            continue
        if patch_usb_2_detect in entry and ios_tid == 58: # Special case, IOS 58 has USB 2.0 "Patch" normally, but don't continue immediately because it may have other patches making it a cIOS
            results.update({ios_tid:(IOSType.ACTIVE, None)})
        if (patch_nand_access_detect in entry) or (patch_eticket_services_detect in entry) or (patch_trucha_patch_detect in entry) or (patch_usb_2_detect in entry and ios_tid != 58):
